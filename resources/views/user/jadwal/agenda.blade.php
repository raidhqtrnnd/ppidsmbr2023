                        ...
                        @for ($i = 0; $i < $jumlah; $i++)
                            <tr>
                                ...
                                <td style="white-space: nowrap;width:20%;word-break: break-word;vertical-align: top; ">
                                    <p style="word-wrap: break-word"> <strong
                                            style="color:rgb(204, 95, 76)"><?php echo wordwrap($data_agenda['data']["$i"]['nama_agenda'], 40, '<br/>'); ?> </strong></p>


                                    @if ($data_agenda['data']["$i"]['status_agenda'] == 'Sedang Berlangsung')
                                        <p> <span style="color:red;"> Status :
                                                {{ $data_agenda['data']["$i"]['status_agenda'] }} </span></p>
                                    @endif
                                    @if ($data_agenda['data']["$i"]['status_agenda'] == 'Belum Berlangsung')
                                        <p> Status : <span style="color:blue;">
                                                {{ $data_agenda['data']["$i"]['status_agenda'] }} </span></p>
                                    @endif
                                ...
                                <td style="width:20%; text-align: center;">
                                    <p> <strong
                                            style="color:rgb(204, 95, 76)">{{ date('d M Y', strtotime($data_agenda['data']["$i"]['tanggal'])) }}
                                        </strong></p>
                                    <p>{{ date('H:i', strtotime($data_agenda['data']["$i"]['jam_mulai'])) }} -
                                        {{ date('H:i', strtotime($data_agenda['data']["$i"]['jam_selesai'])) }}</p>
                                </td>

                                <td style="width:25%; text-align: center;">{{ $data_agenda['data']["$i"]['nama_skpd'] }}
                                </td>
