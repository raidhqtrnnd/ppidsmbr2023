        ...
        <section id="main-container" class="main-container">
            <div class="container">
                                ...
                                <div class="col-md-6 wow slideInUp" data-wow-delay="0.1s">
                                    <div class="post">
                                        <div class="post-media post-image">
                                            ...
                                            <?php }else{ ?>
                                            <img loading="lazy"
                                                src="{{ route('menu.file', encrypt($beritaItem->thumbnail)) }}"
                                                alt="Gambar Content" class="img-fluid"
                ...
                <small class="post-meta-date"><i class="far fa-calendar-alt text-primary me-2"></i><a href="#" style="font-size: 12px;">{{ date('d M Y', strtotime($beritaItem->tgl_post)) }}</a></small>&nbsp; --}}
                                                <small class="me-3"><i class="far fa-user fa-1x text-primary"></i>
                                                    {{ $beritaItem->users->name }}</small>
                                                <small style="margin-left:10px;"><i
                                                        class="far fa-calendar-alt text-primary me-2"></i>
                                                    {{ date('d M Y', strtotime($beritaItem->tgl_post)) }}</small>
                                                <small style="margin-left:10px;"><i
                                                        class="far fa-folder-open text-primary"></i>
                                                    {{ $beritaItem->kategori->nama_kategori }}</small>
                                            </div>
                                            <h2 class="entry-title">
                                                <h6 class="title" style="margin-left: 15px; margin-right:15px;">
                                                    <?php $date = DateTime::createFromFormat('Y-m-d', $beritaItem->tgl_post); ?>
                                                    <a
                                                        href="{{ route('contents_blog', ['year' => $date->format('Y'), 'month' => $date->format('m'), 'day' => $date->format('d'), 'slug' => $beritaItem->slug]) }}">{{ $beritaItem->judul }}</a>
                                                </h6>
                                            </h2>
                                        </div><!-- header end -->
                                        ...
                                        <div class="post-footer" style="margin-left: 15px; margin-right:15px">
                                            <a href="{{ route('contents_blog', ['year' => $date->format('Y'), 'month' => $date->format('m'), 'day' => $date->format('d'), 'slug' => $beritaItem->slug]) }}"
                                                class="btn btn-primary">Baca Lebih Lanjut</a>
                                        </div>
                        ...
                        <div class="sidebar sidebar-right">
                            <div class="sidebar-item search-form">
                                ...
                                <form action="{{ route('reader.search.berita') }}" method="POST" class="mt-3"
                                    ...
                                    <input type="submit" value="Search" style="color:rgb(105, 175, 223);"><i
